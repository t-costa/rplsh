// pipe(generator,farm(sorter) with [ nw: 9],map(multiplier) with [ nw: 1],reduce(sum) with [ nw: 1])
...
public:
	generator_stage() : src(new generator()) {}
	int svc_init() {
		#ifdef TRACE_CORE
...
class map0_stage : public ff_Map<double,double> {
protected:
	multiplier wrapper0;
public:
	map0_stage() : ff_Map(1) {
	...
	double* svc(double *t) {
		double& _task = *t;
		double* out = &_task;
		ff_Map<double,double>::parallel_for(0, _task.size(),[this, &_task](const long i) {
			wrapper0.op(_task[i]);
		},1);
		return out;
	}
...
int main( int argc, char* argv[] ) {
	...
	// farm's worker 7
	sorter_stage _sorter6;
	workers.push_back(&_sorter6);
	
	// farm's worker 8
	sorter_stage _sorter7;
	workers.push_back(&_sorter7);
	
	// farm's worker 9
	sorter_stage _sorter8;
	workers.push_back(&_sorter8);
