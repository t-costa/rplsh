// pipe(generator,farm(sorter) with [ nw: 5],map(multiplier) with [ nw: 3],reduce(sum) with [ nw: 3])
...
public:
	...
	double* svc(std::vector<double>* t) {
		...
		ff_Map<std::vector<double>,double,double>::parallel_reduce(*out, wrapper0.identity,0,_task.size(),bodyF,reduceF,3);
		return out;
	}
};
