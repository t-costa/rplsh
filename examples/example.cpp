...
public:
	...
	matrix* svc(matrix_couple *t) {
		...
		std::cout << "out di map0_stage" << std::endl;
		for (auto v : *out) {
			utils::print_vec(v);
		}
...
public:
	...
	std::vector<utils::elem_type>* svc(std::vector<utils::elem_type> *t) {
		std::cout << "ricevuto da map1" << std::endl;
			utils::print_vec(*t);
...
public:
	...
	std::vector<utils::elem_type>* svc(matrix* t) {
		std::cout <<"input di red0" << std::endl;
		for (auto v : *t) {
			utils::print_vec(v);
		}
		...
		std::cout << "map_matrix_intermedia di red0" << std::endl;
		for (auto v : *mapout) {
			utils::print_vec(v);
		}
